...
func DrawFractal(zoom, moveX, moveY float64, z float64, max_it int, isColor bool) {
	...
	foregroundColors := map[int]string{1: "\x1b[40m", 2: "\x1b[100m", 3: "\x1b[41m", 4: "\x1b[101m", 5: "\x1b[44m", 6: "\x1b[43m"}
			...
			for col := 0; col <= height; col++ {
				...
				if i < max_it {
					if i > 6 {
						...
						if ch, ok := charTable[i]; ok {
							if isColor {
								fmt.Fprintf(Screen, "%s%s", ch, foregroundColors[i])
							} else {
								fmt.Fprintf(Screen, "%s", ch)
							}
						}
					}
				} else {
